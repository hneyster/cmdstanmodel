...
transformed data {
 ...
 array[H] int zetaa = { 30, 3, 17, 13, 23, 11, 19 };
 matrix[R,H] Emod; 
 for (i in 1:R){
   Emod[i,:] = to_row_vector(diagonal(E2[i,zetaa,:]));
 }
...
model {
  ...
  for (i in 1:R) {
      ...
      for (j in 1:(K - max_y2[i] + 1)){
           lp2[j] = poisson_log_lpmf(max_y2[i] + j - 1 |Emod[i,]*beta + E2[i,1,4]*betab[1] + E2[i,3,5]*betab[2]) 
...
generated quantities{
 ...
 // int PPC[R,S];
  ...
  for (i in 1:R){
      N2[i] = poisson_log_rng(Emod[i,]*beta + E2[i,1,4]*betab[1] + E2[i,3,5]*betab[2]);
